...
class InputTextEncoder(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.pos_embed = nn.Parameter(torch.randn(1, self.config['max_len'], self.config['embed_dim']))
  ...
  def tokenize(self, sentence):
    ...
    return torch.tensor(token_sequence)
  
  def forward(self, x):
    
    '''
    Function to get embeddings for token ids
    
    :param self: Description
    :param x: Shape : (Batch, Max_len) : Input Tensor of IDs as generated by tokenizer
    '''
    x = self.embedding(x)
